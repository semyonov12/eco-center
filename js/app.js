...
burger == null ? void 0 : burger.addEventListener("click", function() {
  menuOpen();
});
closeMenu == null ? void 0 : closeMenu.addEventListener("click", function() {
  menuClose();
});
...
class Popup {
  ...
  _openToHash() {
    ...
    if (window.location.hash) {
      classInHash = document.querySelector(`.${window.location.hash.replace("#", "")}`) ? `.${window.location.hash.replace("#", "")}` : document.querySelector(`${window.location.hash}`) ? `${window.location.hash}` : null;
    }
